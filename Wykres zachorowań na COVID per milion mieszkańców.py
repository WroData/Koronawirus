...
###############################################################################
#ustalenie daty maksymalnej w zbiorze
Dane['DATA'] = pd.to_datetime(Dane['dateRep'], format='%d/%m/%Y')

#usunięcie krajow ponizej 1 mln mieszkańców
Dane = Dane[Dane['popData2018'] > 1000000]
...
#wyliczenie tych metryk na mieszkanca
Dane_sort['cumsum_Cases_per_Milion']  = Dane_sort['cumsum_Cases']  / Dane_sort['popData2018'] * 1000000
Dane_sort['cumsum_Deaths_per_Milion'] = Dane_sort['cumsum_Deaths'] / Dane_sort['popData2018'] * 1000000
...
country_show_cases_now = Dane_sort[Dane_sort['countriesAndTerritories'] == show_country]['cumsum_Cases_per_Milion'].max()



#dodanie kolumny roboczej 
Dane_sort.loc[Dane_sort['cumsum_Cases_per_Milion'] <  country_show_cases_now, 'tmp'] = -1
Dane_sort.loc[Dane_sort['cumsum_Cases_per_Milion'] >= country_show_cases_now, 'tmp'] =  1 

#wybranie tych poniżej, żeby wyliczyć minusowe Nday
under = Dane_sort.loc[Dane_sort['cumsum_Cases_per_Milion'] <  country_show_cases_now]
...
#wybranie tych poniżej, żeby wyliczyćdodatnie nday
above = Dane_sort.loc[Dane_sort['cumsum_Cases_per_Milion'] >=  country_show_cases_now]
#posortowanie w odwrotnej kolejnosci
...
#lista krajów, które mają wiecej niż w Polsce
...
#ograniczenia do osi
y_max = tmp[tmp['N_day'] == days_future]['cumsum_Cases_per_Milion'].max()

#wybranie 3 najgorszych scenariuszy
tmp_TOP = tmp[tmp['N_day'] == days_future - 1].sort_values('cumsum_Cases_per_Milion',ascending = False).groupby('N_day').head(4)

#wybranie 3 najlepszych scenariuszy
tmp_BOTTOM = tmp[tmp['N_day'] == days_future - 1].sort_values('cumsum_Cases_per_Milion',ascending = True).groupby('N_day').head(3).sort_values('cumsum_Cases',ascending = False)
...
for country in COUNTRIES_above:
    ...
    if country == show_country:
        print(country)
        plt.plot('N_day', 'cumsum_Cases_per_Milion', data=tmp2,  label = 'Polska',\
                 color='skyblue', linewidth=4, alpha=1)
    #top złe kraje
    elif any(tmp_TOP  ['countriesAndTerritories'].str.contains(country)):
        print(country)
        plt.plot('N_day', 'cumsum_Cases_per_Milion', data=tmp2,  label = 'Największe wzrosty' if i == 0 else "",\
                 color='red',    linewidth=1, alpha=0.3)
        i = i + 1
    #top dobre kraje
    elif any(tmp_BOTTOM['countriesAndTerritories'].str.contains(country)):
        print(country)
        plt.plot('N_day', 'cumsum_Cases_per_Milion', data=tmp2,  label = 'Najmniejsze wzrosty' if j == 0 else "",\
                 color='green',    linewidth=1, alpha=0.3)
        j = j + 1
        
    else:
        plt.plot('N_day', 'cumsum_Cases_per_Milion', data=tmp2, label = '_nolegend_',\
...
tmp4 = tmp4.groupby('N_day')[['cumsum_Cases_per_Milion']].median().reset_index()
tmp4.columns
tmp4['Odpowiednik_Polski'] = tmp4['cumsum_Cases_per_Milion'] * 37978548.0 / 1000000

plt.plot('N_day', 'cumsum_Cases_per_Milion', \
         data=tmp4, \
         label = 'Mediana',\
...
# legenda     
plt.legend()
reorderLegend(axes, ['Polska', 'Największe wzrosty', 'Najmniejsze wzrosty', 'Mediana'])
    
axes.set(title="Dynamika zachorowań COVID-19 na świecie")
# add labels to the axes
axes.set(xlabel="Ilość dni od " + str(round(country_show_cases_now, 0 )) + \
         " zachorowań na milion mieszkańców \n(liczba zachorowań w Polsce na " + \
         str(DATA.strftime("%Y-%m-%d")) + \
         " )" , \
         ylabel="Ilość osób zdiagnozowanych z COVID-19\n na milion mieszkańców");


plt.savefig(os.getcwd() + '\\Wykres  per Milion' + '.png', \
